    ...
    fun handleEvent(event: Event): Requests {
        ...
        when (event) {
            ...
            is Event.Connect -> {
                ...
                error = null
                val id = nextId++
                pendingRequests[id] = PendingRequest.Connect
                requests.add(
                    Request(id, Effect.Http(makeGetRequest("${serverUrl}/global/health"))),
                )
            }
            is Event.LoadSessions -> {
                ...
                requests.add(
                    Request(id, Effect.Http(makeGetRequest("${serverUrl}/session"))),
                )
            }
...
internal sealed class PendingRequest {
    data object Connect : PendingRequest()
